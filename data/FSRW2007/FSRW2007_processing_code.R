...
# Frank et al. (2007) Testing Computational Models of Dopamine and Noradrenaline Dysfunction in Attention Deficit/Hyperactivity Disorder
...
setwd(raw_path)
# Read dataset
# Paper: Eighteen ADHD participants and 21 controls
ds_adhd <- read.csv(file = "PS_merge_ADHD.txt", header = TRUE, stringsAsFactors = FALSE)
ds_control <- read.csv(file = "PS_merge_ADHD_ctl.txt", header = TRUE, stringsAsFactors = FALSE)
option <- c("A", "B", "C", "D", "E", "F")
...
process_st <- function (ds, group) {
  ...
  process_dfs <- function (dfs, session) {
    ...
    if (group == "ADHD") {
      condition <- 1
      note2 <- ifelse(dfs$Drug == 1, paste0("ADHD on medication ", dfs$Running),paste0("ADHD off medication ", dfs$Running))
    } else {
      condition <- 2
      note2 <- paste0("Control ", dfs$Running)
    }
    ...
    problem <- ifelse(dfs$Running == "TrainInterleaved", 1, 2)
    stage <- education <- age <- sex <- NA
...
ds_final <- rbind(process_st(ds = ds_adhd, group = "ADHD"), process_st(ds = ds_control, group = "Control"))
