...
validation_df = pd.DataFrame(columns=['study', 'high_mean_choices', 'high_mean_choices_problems', 'high_mean_choices_problems_detail',
...
# filter data
studies_list = df['study'].str.split('_').str[0]
    ...
    for data_path in data_name_list:
        ...
        for option_pair in option_pairs:
            ...
            if not data_partial.empty:
                ...
                data_partial_num = data_partial
                # high mean choices
                participants = data_partial_num['subject'].unique()
                ignore_trial = 0
                for partiaipant in participants:
                    data_partial_num_participant = data_partial_num[
                        data_partial_num['subject'] == partiaipant].sort_values('trial').reset_index(drop=True)
            ...
            if not data_full.empty:
                ...
                if len(data_full_num)- equal_mean_choices_num_p - len(participants) == 0:
                    ...
                    high_mean_choices_test = binomtest(high_mean_choices_num_p, (len(data_full_num) - equal_mean_choices_num_p - len(participants)), p=1/option_number, alternative='greater')
                    if high_mean_choices_test.pvalue <= alpha:
                        high_mean_choices_problems_num.append(1)
                    else:
                        high_mean_choices_problems_num.append(0)
        ...
        new_row = [study,round(np.nansum(high_mean_choices_num) / (len(cleaned_data)-len(equal_mean_choices_num)-ignore_trials_num - participants_num), 3),
