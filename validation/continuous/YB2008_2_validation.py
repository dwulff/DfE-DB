...
option_ev = dict(A=-10, B=-20, C=-9, D=-10, E=-25, F=-100)
sqrOption = dict(A=20, B=20, E=17.7, F=354)
optionA = np.random.normal(loc=option_ev['A'], scale=sqrOption['A'], size=5000)
optionA = optionA[(optionA > -40) & (optionA < 20)]
if len(optionA) > 1000:
...
optionB = optionB[(optionB > -50) & (optionB < 10)]
if len(optionB) > 1000:
    optionB = np.random.choice(optionB, size=1000, replace=False)
optionB = np.round(optionB, -1)
optionD = [-10] * 1000
optionC = np.concatenate((np.array([-20]*900), np.random.uniform(85, 95, 100)), axis=0)
optionE = np.random.normal(loc=option_ev['E'], scale=sqrOption['E'], size=5000)
optionE = optionE[optionE < 0]
if len(optionE) > 1000:
    optionE = np.random.choice(optionE, size=1000, replace=False)
...
option_range = dict(A=[-40, 20], B=[-50, 10], C=[-20, 95], D=[-10], E=[option_ev['E']-3*sqrOption['E'], 0], F=[option_ev['F']-3*sqrOption['F'], option_ev['F']+3*sqrOption['F']])
option_ev = dict(A=-10, B=-20, C=-9, D=-10, E=-25.6, F=-100)
