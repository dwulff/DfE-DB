...
option_ev = dict(A=20, B=10, C=10, D=9, E=100, F=25)
sqrOption = dict(A=20, B=20, E=354, F=17.7)
optionA = np.random.normal(loc=option_ev['A'], scale=sqrOption['A'], size=5000)
optionA = optionA[(optionA > -10) & (optionA < 50)]
if len(optionA) > 1000:
    optionA = np.random.choice(optionA, size=1000, replace=False)
optionA = np.round(optionA, -1)
optionB = np.random.normal(loc=option_ev['B'], scale=sqrOption['B'], size=5000)
optionB = optionB[(optionB > -20) & (optionB < 40)]
if len(optionB) > 1000:
    optionB = np.random.choice(optionB, size=1000, replace=False)
optionB = np.round(optionB, -1)
optionC = [10] * 1000
optionD = np.concatenate((np.array([20]*900), np.random.uniform(-95, -85, 100)), axis=0)
optionE = np.random.normal(loc=option_ev['E'], scale=sqrOption['E'], size=1000)
optionF = np.random.normal(loc=option_ev['F'], scale=sqrOption['F'], size=3000)
optionF = optionF[optionF > 0]
if len(optionF) > 1000:
    optionF = np.random.choice(optionF, size=1000, replace=False)
option_inf = dict(A=optionA, B=optionB, C=optionC, D=optionD, E=optionE, F=optionF)
option_range = dict(A=[-10, 50], B=[-20, 40], C=[10], D=[-95, 20], E=[option_ev['E']-3*sqrOption['E'], option_ev['E']+3*sqrOption['E']], F=[0, option_ev['F']+3*sqrOption['F']])
option_ev = dict(A=20, B=10, C=10, D=9, E=100, F=25.6)
