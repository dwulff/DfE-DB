...
option_number = 3  # option numbers for each problem. To calculate the proportion should get to choose high ev option significantly higher than chance.
# create empty file
wb = Workbook()
ws = wb.active
...
# option_file = study + '_options.csv'
...
def split_data(data):
    ...
    data_sort = data.sort_values(by='reward')
    top_50 = data_sort.head(int(0.5 * len(data_sort)))
    tail_50 = data_sort.tail(int(0.5 * len(data_sort)))
    return top_50['reward'], tail_50['reward']

option_ev = dict(A=-0.5, B=0.5, C=0.5, D=0.5, E=1, F=1, G=2.5, H=2.5, I=2.5, J=2.5, K=3, L=3.5)
optionA = np.random.uniform(0, 1, 1000)
optionB = add_num(1, -1)
optionC = add_num(2, -2)
optionD = np.random.uniform(0, 1, 1000)
optionE = add_num(2, -1)
optionF = add_num(4, -2)
optionG = 2 + np.random.uniform(0, 1, 1000)
optionH = add_num(1, 3)
optionI = add_num(0, 4)
optionJ = 2 + np.random.uniform(0, 1, 1000)
optionK = add_num(1, 4)
optionL = add_num(0, 6)
option_inf = dict(A=optionA, B=optionB, C=optionC, D=optionD, E=optionE, F=optionF, G=optionG, H=optionH, I=optionI, J=optionJ, K=optionK, L=optionL)
option_range = dict(A=[0, 1], B=[-1, 0, 1, 2], C=[-2, -1, 2, 3], D=[0, 1], E=[-1, 0, 2, 3], F=[-2, -1, 4, 5], G=[2, 3], H=[1, 2, 3, 4], I=[0, 1, 4, 5], J=[2, 3], K=[1, 2, 4, 5], L=[0, 1, 6, 7] )
    ...
    if not data_partial.empty:
        ...
        for k in range(0, len(options)):
            ...
            try:
                ...
                continue
            if len(option_range_sub) > 2:
                if all(option_range_sub[0] <= x <= option_range_sub[1] or option_range_sub[2] <= x <= option_range_sub[3] for x in set1):
        ...
        for k in range(0, len(options)):
            ...
            if len(option_range_sub) > 2:
                if all(option_range_sub[0] <= x <= option_range_sub[1] or option_range_sub[2] <= x <= option_range_sub[3] for x in set1):
                    ...
                    print(min(set1), max(set1), option_range_sub)
                    choice_merge_option = choice_merge_option[
                        (choice_merge_option['reward'] >= min(option_range_sub)) & (
                                    choice_merge_option['reward'] <= max(option_range_sub))]
                    ...
                    for a in range(0, len(choice_list)):
                        ...
                        data_a_clean = data_a[(data_a[reward_list[a]] >= min(option_range_sub)) & (
                                    data_a[reward_list[a]] <= max(option_range_sub))]
